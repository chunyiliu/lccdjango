...
def addtocart(request,ctype=None,productid=None):
    ...
    if ctype=="add": #將商品加入至購物車中
        product=Goods.objects.get(id=productid) #會用get 是因為帶入的產品ID,一定有在資料表中
        ...
        return redirect('/cart/') #跳轉指此網址 (跳到下面的 cart)
    
    elif ctype=="update": #修改購物車數量
        n=0
        for unit in cartlist: #將購物車的內容抓出來,並修改數量和總價
            amount=request.POST.get('qty'+str(n),'1') #抓取qty0,qty1,qty2 預設數量是1
            if len(amount)==0:
                amount='1'
            elif int(amount)<=0:
                amount='1'
...
def cartok(request): #已確認資料並送出的,所以會將訂單寫入到資料庫
    ...
    if payType == '信用卡':
    #return render(request,'cratok.html',locals())
